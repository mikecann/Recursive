var ContextMenuController = (function () {
    ...
    ContextMenuController.prototype.showContext = function (forCrawler, x, y) {
        ...
        items.push({
            label: 'View Files (' + numFiles + ')',
            icon: 'images/documents-stack.png',
            action: function () {
                filesModal.open(forCrawler);
            }
        });
        items.push({
            label: 'Open Pages (' + crawlersForThisHost.length + ')',
            icon: 'images/ui-tab--arrow.png',
            action: function () {
                crawlersForThisHost.forEach(function (c) {
