 ...
 */
jQuery.fn.contextPopup = function(menuData,xpos,ypos) {
  ...
  function createMenu() {
	...
	settings.items.forEach(function(item) {
	  if (item) {
		...
		if (item.action) {
		  row.find('a').click(function(){ item.action(); });
		}
	  } else {
	...
	var menu = createMenu()
	.show()
	.css({ zIndex: 1000001, left: menuData.pageX + 5 /* nudge to the right, so the pointer is covering the title */, top: menuData.pageY })
